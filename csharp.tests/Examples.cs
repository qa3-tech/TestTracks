...
public static class Examples
{
  ...
  static readonly List<SuiteResult> MockResults = [
         new SuiteResult("TestSuite", [
            new TestOutcomeResult(TestOutcomeKind.Passed, "test1", 10.0, [], null, null),
            new TestOutcomeResult(TestOutcomeKind.Failed, "test2", 20.0, [new TestError("expected 1", 1, 2)], null, null),
            new TestOutcomeResult(TestOutcomeKind.Skipped, "test3", 0, [], "not implemented", null)
        ], 100.0)
     ];

  public static TestTracks.TestSuite JUnit => Suites.Create("JUnitXml (C#)",
      Tests.Create("contains xml declaration", () =>
      {
        var xml = Printer.ToJUnitXml(MockResults);
        return Asserts.Contains("<?xml version", xml, "should have xml declaration");
      }),
      Tests.Create("contains testsuites with counts", () =>
      {
        var xml = Printer.ToJUnitXml(MockResults);
        return Outcomes.All(
              Asserts.Contains("tests=\"3\"", xml, "should have test count"),
              Asserts.Contains("failures=\"1\"", xml, "should have failure count"),
              Asserts.Contains("skipped=\"1\"", xml, "should have skipped count")
          );
      }),
      Tests.Create("contains testcase elements", () =>
      {
        var xml = Printer.ToJUnitXml(MockResults);
        return Outcomes.All(
              Asserts.Contains("<testcase name=\"test1\"", xml, "should have passed test"),
              Asserts.Contains("<failure message=", xml, "should have failure element"),
              Asserts.Contains("<skipped message=", xml, "should have skipped element")
          );
      }),
      Tests.Create("writes to file", () =>
      {
        var path = Path.GetTempFileName();
        try
        {
          Printer.WriteJUnitXml(MockResults, path);
          var content = File.ReadAllText(path);
          return Outcomes.All(
              Asserts.NotEmpty(content, "file should have content"),
              Asserts.Contains("<testsuites", content, "should be valid junit xml")
          );
        }
        finally
        {
